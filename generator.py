...
class RigAnythingMotionGenerator(BaseGenerator):
    ...
    def is_downloaded(self):
        ...
        rig_done = (rig_dir / "config.yaml").exists()
        motion_done = (motion_dir / "latest.ckpt").exists() or (motion_dir / "pytorch_model.bin").exists()
        
        return rig_done and motion_done
    ...
    def generate(self, model_bytes, params, progress_cb=None, cancel_event=None):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            try:
                ...
                filename = "riganimated_%d_%s.glb" % (int(time.time()), uuid.uuid4().hex[:8])
                ...
                print("[RigAnything] Error during processing: %s" % e)
                raise
    ...
    def _rig_mesh(self, mesh_path, simplify_flag, target_faces, output_dir, progress_cb, cancel_event):
        ...
        if simplify_flag:
            ...
            if self._rig_utils and self._rig_utils['mesh_simplify']:
                self._rig_utils['mesh_simplify'].simplify_mesh(
                    str(mesh_path), 
                    str(simplified_path), 
                    target_faces
                )
        ...
        if self._rig_utils and self._rig_utils['rig_pipeline']:
            self._rig_utils['rig_pipeline'].rig_mesh(
                str(simplified_path),
                str(rigged_path)
            )
        else:
